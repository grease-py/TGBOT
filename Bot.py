...
def analyze_wallet(update, context):
    ...
    # Create a hyperlink to the wallet address on Solana Explorer
    wallet_link = f"https://explorer.solana.com/address/{wallet_address}"

    analysis = f"""
Wallet Analysis for [{wallet_address}]({wallet_link})
...
Total Transactions Analyzed: {len(transactions)}
    ...
    update.message.reply_text(analysis, parse_mode='Markdown', disable_web_page_preview=True)
